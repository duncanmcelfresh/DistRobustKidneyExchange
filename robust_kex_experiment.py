...
from kidney_ip import (
    OptConfig,
    optimise_robust_picef,
    optimize_picef,
    optimize_SAA_picef,
    optimize_DRO_SAA_picef,
)
...
def robust_kex_experiment(args):
    ...
    for digraph, ndd_list, graph_name in graph_generator:
        ...
        # # REMOVE THIS
        # # save LP file
        # lp_dir = "/Users/duncan/research/"
        # _, _, m = optimize_picef(OptConfig(digraph, ndd_list, args.cycle_cap,
        #                          args.chain_cap,
        #                          verbose=args.verbose))
        # m.write(os.path.join(lp_dir, graph_name + ".lp"))
        # continue
        ...
        for i_trial in range(args.num_trials):
            ...
            for alpha in args.alpha_list:
                ...
                if args.use_truemean:
                    sol_dict["nonrobust_truemean"] = optimize_picef(
                        OptConfig(
                            digraph,
                            ndd_list,
                            args.cycle_cap,
                ...
                if args.use_samplemean:
                    ...
                    sol_dict["nonrobust_samplemean"] = optimize_picef(
                        OptConfig(
                            digraph,
                            ndd_list,
                            args.cycle_cap,
                ...
                if args.use_saa:
                    for saa_gamma in args.saa_gamma_list:
                        for saa_alpha in args.saa_alpha_list:
                            sol_dict[
                                (
                                        "saa_gamma_%s_alpha_%s"
                                        % (str(saa_gamma), str(saa_alpha))
                                )
                            ] = optimize_SAA_picef(
                                OptConfig(
                                    digraph,
                ...
                if args.use_dro_saa:
                    for theta in args.dro_theta_list:
                        ...
                        weights = [wt for e in pair_e_list for wt in e.weight_list] + [
                            wt for e in ndd_e_list for wt in e.weight_list
                        ]
                        w_min = min(weights)
                        w_max = max(weights)
                        for saa_gamma in args.saa_gamma_list:
                            for saa_alpha in args.saa_alpha_list:
                                sol_dict[
                                    (
                                            "dro_saa_gamma_%s_alpha_%s_theta_%s"
                    ...
                    for i_realization in range(args.num_weight_realizations):
                        ...
                        for noise_scale in args.noise_scale_list:
                            ...
                            if args.use_omniscient:
                                ...
                                sol_dict["omniscient"] = optimize_picef(
                                    OptConfig(
                                        digraph,
                                        ndd_list,
                                        args.cycle_cap,
                            ...
                            for sol_name, (sol, matched_edges) in sol_dict.items():
                                ...
                                # get the matching composition
                                num_cycles = {i: 0 for i in [2, 3, 4]}
                                num_chains = {i: 0 for i in [0, 1, 2, 3, 4, 5]}

                                for cycle in sol.cycle_obj:
                                    num_cycles[len(cycle)] += 1
...
def realize_edge_weights(digraph, ndd_list, rs, noise_scale=0.0):
    ...
    for e in digraph.es:
        if rs.rand() < 0.5:
            noise = -0.5 * noise_scale
            # noise = noise_scale
        else:
            noise = -noise_scale
        e.weight = max(e.draw_edge_weight(rs) + noise, 0.01)
    for n in ndd_list:
        for e in n.edges:
            if rs.rand() < 0.5:
                noise = -0.5 * noise_scale
...
def main():
    ...
    if args.DEBUG:
        ...
        arg_str += (
            " --input-dir /Users/duncan/research/graphs/graphs_from_john/graphs_64"
        )
