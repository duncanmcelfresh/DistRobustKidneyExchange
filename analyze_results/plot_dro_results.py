...
# remove all dro or saa rows with gamma not equal to this value
df = df.loc[~df['dro_gamma'].isin([0.1, 1.0])]
...
id_columns = ['graph_name', 'trial_num', 'alpha', 'alpha', 'cycle_cap', 'chain_cap', 'noise_scale']
...
df_clean['noise_scale'] = df_clean['uid'].apply(lambda x: x[6])
